...
# containing the executable, installer, uninstaller, and Quick Action workflow.
# Run on macOS only.
#
# Usage:
...
cp -R "$INSTALL_MACOS/$WORKFLOW_NAME" "$STAGING_DIR/"
...
Use:
  Right-click any PDF in Finder → Quick Actions → "Compress PDF to 30MB"
...
If the Quick Action does not appear:
  System Settings → Privacy & Security → Extensions → Finder → enable "Compress PDF to 30MB"
